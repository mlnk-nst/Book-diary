...
try {
    ...
    $stmtCheck->execute([$userId, 'reading', $bookId]);
    if ($stmtCheck->fetch()) {
        echo json_encode(['success' => false, 'message' => 'Бали за статус "Читаю" вже нараховані для цієї книги']);
        exit;
    }
    ...
    $stmtLog->execute([$userId, 'reading', $bookId, $xpToAdd]);
